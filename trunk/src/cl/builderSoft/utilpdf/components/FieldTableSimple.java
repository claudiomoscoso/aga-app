...
public class FieldTableSimple extends PdfComponent {
	...
	public FieldTableSimple() {
		this(null, null);
	}
	
	public FieldTableSimple(List labels) {
		this(labels, null);
	}
	
	public FieldTableSimple(List labels, Object values[]) {
	...
	public Element render() {
		try {
            ...
            int aux = 0;
			for(int i=0, v=0; i<labels.size(); i++){
				Object row[] = (Object[])labels.get(i);
				for(int j=0, col=0; col<row.length+2;col++) {
//					int border = Rectangle.LEFT | Rectangle.RIGHT;
//					if (j==row.length) border |= Rectangle.RIGHT;
                    int border = Rectangle.NO_BORDER;
                    if (j == 0 ) border |= Rectangle.LEFT;
                    if (col == row.length + 1) border |= Rectangle.RIGHT;
					...
					if (aux==valueColumn) {
						...
						p = new com.lowagie.text.Paragraph(row[j].toString(),Resources.fontBold);
						cell = new PdfPCell(p);
