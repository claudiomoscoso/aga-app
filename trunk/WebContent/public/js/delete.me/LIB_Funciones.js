...
*/
	keyASCII = window.event.keyCode;
//	alert(keyASCII);
	if(keyASCII == 47 || keyASCII == 46)
	{
		window.event.keyCode = 45;
		keyASCII = 45;
	}
//	alert(keyASCII);
	if( window.event.srcElement.value.length >= 10 || keyASCII != 45 && keyASCII != 13 && keyASCII < 48 || keyASCII > 57 )
	{
	   window.event.keyCode = '';
	}
}
...
function setAnchoControls(arrControls)
{
	...
	for(var I = 0 ; I <= arrControls.length-1; I++)
	{
...
//--------------------------------------
function CargaCombo(cmbCombo, xmlDato)
{
	var lstLista;
	lstLista = xmlDato.XMLDocument.selectNodes('salida/registro');
			
	for(var i=0;i!=lstLista.length;i++)
	{
		option = document.createElement("OPTION");
		option.value = lstLista.item(i).selectSingleNode('Codigo').text;
		option.text = lstLista.item(i).selectSingleNode('Detalle').text;
