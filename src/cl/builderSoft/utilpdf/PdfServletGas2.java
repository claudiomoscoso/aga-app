...
public class PdfServletGas2 extends PdfServlet {
	...
	private void setComponentsValues(List componentesList, CertificadoGasVo certificadoVo) {
		...
		for (int i = 0; i < size; i++) {
			...
			componenteVo.setEspecificacion(componenteElement.element("cAnalisis").getTextTrim());
	...
	private void setCertificadoValues(String idSolicitud, Connection conn, BSExecutor exec, CertificadoGasVo certificadoVo,
		...
		String nlote = solicitudElement.element("cLoteAGA").getTextTrim();
		certificadoVo.setNumeroLote(nlote.length() < 9 ? nlote : nlote.substring(nlote.length()-9, nlote.length()));
